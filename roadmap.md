...
- **UI 設計 v1.4 と実装の差分** (2026-07-06、モック v2.1→v2.2 レビューで確定した改修バックログ。
  ...
  ⑤**配信の画質/FPS ピッカー (Discord 準拠、v1.4 追加)** — 画面共有開始時に解像度と FPS を
  選択 (既定は最高 4K60)。現実装は src/livekit/options.ts の固定値 → 選択値を captureOptions /
  publishDefaults に反映する UI を追加 (requirements.md §3。Sonnet M5 の「現状維持」判断を上書き)
