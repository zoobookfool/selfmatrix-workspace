...
## E2E: 2 ユーザー通話 + 配信 + 窓移動無再接続 + 7 語彙実 DOM 検証 + 実クリック検証 + 通話跨ぎ回帰 (M1 step 3c-2/3c-3 + M1 全体レビュー即時対応)
...
M1 全体レビュー (Fable + GPT) の即時修正で 2 セクション追加済み (下記 pass 条件の
`realClickVocabulary`/`callRespawn`、詳細は各専用節):

- **realClickVocabulary** (test-critical #3 対応): 既存の `callControlVocabulary` は 7 語彙のうち
  6 つを `__selfmatrixE2E.invokeCallControl()` (main 直叩きの E2E 専用窓口) で駆動しており、cinny 側
  の本番配線 (`CallControls.tsx` の onClick → `NativeCallControl.toggleX()` →
  `transport.callControlInvoke()`) を経由していなかった。spotlight/emphasis/settings/sound を
  cinny 自身の実ボタンへの実クリックで駆動し直す。
- **callRespawn** (GPT レビュー P1b 回帰確認): alice が実クリックで通話を退出し、同じプロセスで
  再度参加する「通話跨ぎ」サイクルを駆動し、C1 (`registerPreloadScript` の二重登録防止) が
  2 通話目でも保たれていることと、claim-once キャッシュ (G2) が 2 通話目で例外を投げないことを
  実測する。
...
- **callControlVocabulary**: 7 語彙 (`toggleScreenshare`/`toggleSpotlight`/`toggleEmphasis`/
  ...
  にも反映されることを確認する。**この語彙は main 直叩きの E2E 専用窓口を使うため、cinny 自身の
  onClick → `NativeCallControl` 配線は経由しない** (下記 `realClickVocabulary` 節参照)。
- **realClickVocabulary** (M1 全体レビュー test-critical #3 対応、下記専用節参照): spotlight/
  emphasis/settings/sound を cinny 自身の実ボタンへの実クリックで駆動し、`callControlVocabulary`
  が経由しない本番配線 (`CallControls.tsx` の onClick → `NativeCallControl.toggleX()` →
  `transport.callControlInvoke()`) を実際に検証する。
...
- **callRespawn** (GPT レビュー P1b 回帰確認、下記専用節参照): alice が cinny 自身の切断ボタンで
  実クリック退出し、同じプロセスで再度参加する「通話跨ぎ」サイクルが、call view 破棄・
  registerPreloadScript 登録カウント不変・claim-once キャッシュ・screenshare 単発反転のすべてで
  健全であること。
...
### realClickVocabulary: cinny 自身の実 UI クリックによる語彙検証 (M1 全体レビュー test-critical #3 対応)

M1 全体レビュー (Fable) で、`callControlVocabulary` (上記) が検証している 7 語彙のうち 6 つは
alice 側の claim 済み transport から `__selfmatrixE2E.invokeCallControl()` (main プロセスを
直叩きする E2E 専用の窓口) で駆動しており、**cinny 自身の本番配線**
(`CallControls.tsx`/`Controls.tsx` の onClick → `NativeCallControl.toggleX()` →
`transport.callControlInvoke()`) を一切経由していない — 実クリックで検証済みだったのは
`verifyMidCallSettingsSync()` の screenshare だけだった、という指摘が入った。

`runRealClickVocabulary()` は残る 4 対象を alicePage 上の cinny 自身のボタンへの実クリックで駆動し、
call-control-preload.cjs 経由の実 DOM 到達と `onCallControlState` push による cinny 自身の再同期
(aria-pressed の反転) を実測する。各対象を `{clicked, stateConfirmed}` として evidence に記録し、
4 対象全ての AND が `realClickVocabulary.pass` になる (全体 `result.pass` にも組み込み済み)。

- **対象外の 2 つ**: screenshare は `verifyMidCallSettingsSync()` で既に実クリック検証済みのため
  対象外。reactions は cinny 側 (`CallControls.tsx`) にボタン自体が存在しない (実コード確認済み —
  上記「既知の環境ギャップ: `toggleReactions`」節参照) ため対象外。
- **使用したセレクタ (すべて実コードで確認したもの、推測なし)**:
  - spotlight: `[data-testid="call_layout_toggle"]` (既存)
  - emphasis: `[data-testid="call_emphasis_toggle"]` (既存、spotlight=false のときのみ cinny 側
    DOM に存在)
  - settings: `[data-testid="call_menu"]` (既存、PopOut を開く) →
    `[data-testid="call_menu_settings"]` (**このコミットで cinny 側の `MenuItem` に追加** —
    元は testid が無く実クリックで安定して選択できなかった)
  - sound: `[data-testid="call_control_sound"]` (**このコミットで cinny 側の `SoundButton`
    (`Controls.tsx`) に追加** — 元は testid も `aria-pressed` も無かった)
  - hangup (callRespawn 節で使用): `[data-testid="call_hangup"]` (**このコミットで cinny 側の
    切断ボタン (`CallControls.tsx`) に追加** — 元は testid が無かった)
- **判定方法**:
  - spotlight/emphasis/sound: `onCallControlState` push (`main.cjs` の `callControlMessages`)
    の対応フィールドと、cinny 自身の DOM (`aria-pressed`) の両方が実際に反転したことを確認する
    (settings は元実装どおり push を伴わないため対象外 — `callControlVocabulary` 節と同じ理由)。
  - settings: `call_menu` → `call_menu_settings` の実クリックで EC 側の設定ダイアログを開き、
    call view 側の `[role="dialog"]` 出現を確認してから Escape で閉じ、消えたことも確認する。
  - sound: 2 ユーザー通話中は bob の `<audio>` 要素が call view にあるはずなので実クリックで
    off→on を検証する。**audio 要素が無い場合は `computeSoundState()`
    (`call-control-preload.cjs`) が push に `sound` フィールドを含めないため、判定から除外して
    よい** — その場合 `skipped:true`/`skipReason` を evidence に残し、その対象は自動的に
    `pass:true` (AND に影響しない) として扱う。実測ではこのプロトタイプの 2 ユーザー通話では
    毎回 audio 要素が存在し、実際に off→on の実クリック判定まで到達している。
- **変異検証で確認した効果**: cinny 側 `NativeCallControl.toggleSpotlight()` が
  `transport.callControlInvoke()` を呼ばないよう変異させると、`callControlVocabulary`
  (main 直叩き経路) は無傷で PASS したままだが `realClickVocabulary` の spotlight が FAIL する
  (push が来ない/`aria-pressed` が変化しない) ことを実測で確認した — 詳細は本 README 末尾の
  変異検証記録を参照。

### callRespawn: 通話跨ぎ回帰確認 (GPT レビュー P1b 対応)

GPT レビュー P1b は「通話跨ぎ (join → hangup → 再 join) の回帰確認」を求めた。C1
(`main.cjs` の `callViewPreloadRegistrationCount` コメント参照。M1 全体レビュー即時修正で
`session.fromPartition(...).registerPreloadScript()` の累積登録バグを修正済み) と G2
(`nativeBridge.ts` の `getOrClaimWidgetTransport()` コメント参照。claim-once キャッシュで 2 通話目の
`NativeCallEmbed` 構築が throw しないよう修正済み) はどちらも「2 通話目」を前提にした修正だが、
これまで E2E は 1 プロセスにつき 1 通話しか実際に駆動していなかった。

`runCallRespawn()` は既存の 2 ユーザー通話 (screenshare + 窓移動の検証が終わった後) で、alice が
cinny 自身の切断ボタン (`[data-testid="call_hangup"]`、実クリック) で退出し、同じ Electron
プロセスで再度参加するところまでを実際に駆動する (bob は退出させず、そのまま元の通話に残す)。
証跡スクリーンショットの撮影はこのステップより **前** に行う (通話跨ぎ後は screenshare/窓移動前の
状態に戻ってしまうため)。

- **callViewClosed**: 退出後、実際に main 側の call view が破棄される
  (`state.callViewState === "none"`) こと。
- **backToPrescreen / joinButtonEnabled**: cinny 自身の prescreen に戻り、"参加" ボタンが
  可視かつ enabled になること。
  - **つまずき (実機確認)**: 当初は `nativeE2ELib.mjs` の `openVoiceLoungeAndJoin()` (alice/bob の
    初回 join と共通のヘルパー) をそのまま再利用していたが、これは「サイドバーの room 項目を
    毎回クリックしてから join ボタンを探す」実装になっている。alice は退出後も同じ room を
    表示したままなので、このサイドバー再クリックが不要なだけでなく、実機で 2 回連続して
    「join ボタンが 20〜45 秒経っても disabled のまま click() がタイムアウトする」障害を再現した
    (`isEnabled()` の事前チェックが一瞬 true を観測した直後にも起きた — サイドバー再クリックの
    たびに `canJoin` の再計算がリセットされていたと見られる)。`openVoiceLoungeAndJoin()`
    自体は変更せず (alice/bob の初回 join は今までどおり)、`runCallRespawn()` 側でサイドバー
    再クリックを行わず join ボタンだけを直接待ってクリックする形に変更したところ、以後 2 回
    連続で安定して PASS した。
- **再 join 成功自体が claim-once キャッシュ (G2) の証明**: `getOrClaimWidgetTransport()` の
  キャッシュが機能していなければ、2 通話目の `NativeCallEmbed` コンストラクタが
  `bridge.claimWidgetTransport()` の claim-once ガードで同期的に throw し、"参加" クリック自体が
  in-call 状態まで到達しない。`rejoinInCall.pass` (alice-join と同一の 4 条件) が true であること
  自体がこれの実測になる。
- **registrationStable**: `callViewPreloadRegistrationCount` (このコミットで `getSnapshot()` に
  追加 — 従来は main プロセス内部にしかなく E2E から読めなかった) が、退出前・再 join 後の両方で
  `1` のままであること (C1 の E2E レベル回帰確認)。実測: `before=1, after=1`。
- **screenshareSingleToggle**: 再 join 後、cinny 自身の screenshare ボタンを実クリック 1 セット
  (アイコン → 画質/FPS ピッカー → 配信を開始) で操作すると、実際に 1 回だけ反転すること。
  `call-control-preload.cjs` が call view の同一フレームに二重登録されていた場合、1 回の RPC
  往復で実クリックが 2 回発生し「開始→即停止」になる (aria-pressed が反転しない/
  `action-toggleScreenshare` の state push が 2 件になる) — before/after の aria-pressed と
  push 件数の両方で判定する。実測: `before:"false" → after:"true"`、push 件数 `1`。

### realClickVocabulary の変異検証記録 (production 配線の検証になっていることの証明)

`realClickVocabulary` が実際に cinny 側の本番配線を検証していること (`callControlVocabulary` の
main 直叩き経路とは別の、独立したカバレッジであること) を、cinny 側 `NativeCallControl.ts` の
`toggleSpotlight()` を一時的に変異させて実測した:

1. `toggleSpotlight()` の本体を `transport.callControlInvoke()` を一切呼ばない no-op に変異させる
   (バックアップ → 編集)。
2. `cinny` を再 build (`npm run build`、native-prototype はビルド済み dist を読むため必須)。
3. `npm run e2e:callflow` を実行 → **`realClickVocabulary.pass=false`** (spotlight の
   `stateConfirmed:false`, `statePushOn:null`, `afterOn` が `beforeOn` から変化しない) で
   全体 `pass=false`/`EXIT:1` になることを実測。**一方 `callControlVocabulary.pass=true` のまま**
   だった — main 直叩き経路 (`__selfmatrixE2E.invokeCallControl()`) は cinny の
   `NativeCallControl.toggleSpotlight()` を経由しないため、この変異の影響を受けない。
   これが「`realClickVocabulary` が `callControlVocabulary` の見ていない production 配線を
   実際に検証している」ことの実証になる。
4. 変異を復元 (`cp` バックアップから戻し、`cmp` でバイト同一であることを確認)、cinny を再 build。
5. `npm run e2e:callflow` を再実行 → 全体 `pass=true`/`EXIT:0` に復帰することを実測。
