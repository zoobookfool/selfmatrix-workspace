...
| 優先 | 項目 | 状態 | 参照 | 完了条件 |
| --- | --- | --- | --- | --- |
| P0 | Native prototype の Cinny widget host 接続 | M0 must-fix 済み、NativeWidgetTransport / NativeCallHost adapter が次 | [native-client-decision.md](../design/native-client-decision.md), [native-client-rethink.md](../design/native-client-rethink.md), [native-milestones.md](native-milestones.md), [desktop-window-spike.md](../spikes/desktop-window-spike.md) | Cinny 本体の widget host / ClientWidgetApi 相当と `WebContentsView` EC view が接続され、実 join 前の widget action が prototype 上で通る |
| P0 | Electron WebContentsView による通話 view 再親子付け検証 | reparent/displayMedia/loopback/native smoke 済み、M0 evidence commit 済み、最終 LiveKit join 待ち | [native-client-decision.md](../design/native-client-decision.md), [desktop-window-spike.md](../spikes/desktop-window-spike.md), [native-client-rethink.md](../design/native-client-rethink.md) | 実 EC + dev MatrixRTC で join / 共有中 view 移動 / 実 UI からの system audio まで確認し、production release 可否が決まる |
| P0 | ネイティブ化するか web 版別窓開始モードを実装するかの判断 | 完了: native 案 A -> B へ進む。web 版別窓開始は fallback | [native-client-decision.md](../design/native-client-decision.md), [call-window-mode.md](../design/call-window-mode.md), [native-client-rethink.md](../design/native-client-rethink.md) | prototype 結果で native 案 B 継続、または web 版実装再開が決まる |
