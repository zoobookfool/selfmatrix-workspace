...
| ④ | 最上部の Electron メニューバー (File / Edit / View / Window) が英語で、Discord 風には不要 | **native 限定** | `selfmatrix-desktop/src/main.cjs` でアプリメニューを設定していない (Electron 既定の英語 role メニューが出る)。`Menu.setApplicationMenu(null)` で消すか、日本語の最小メニューを定義する | 低 (体裁) | 未対応 |
...
| ⑧ | スピーカーミュートが 1 人の時 ON にできない | **web 共通** | `CallControl.ts:221-229` の `setSound()` は**リモート参加者の `<audio>` 要素を mute** する実装。1 人の時は他人の audio が無く効果が出ない。加えて `CallControl.ts:247-248` に「mic ON かつ sound OFF なら sound を自動で ON に戻す」結合があり、これがトグルを ON に保てない主因の可能性 | 中 | 未対応 |
