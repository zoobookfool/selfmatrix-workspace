...
- 「別ウィンドウ通話開始モード」(call-window-mode.md v1.4、web 前提で設計済み) について。
  案 B が成立すると「最初から別窓で開く」だけでなく「いつでも無再接続で出し入れ」になり、設計の前提
  (再接続を避けるには最初から別窓しかない) が変わる。
  **(2026-07-07 方針変更で更新)** 案 B が成立した (M1 完了) ため、ネイティブ版は **Discord 準拠**に
  戻す (native-milestones M3 が正本)。v1.4 の決定事項のうち引き継ぐのは**「窓サイズ/位置の記憶」と
  「別窓での EC フッター表示」だけ**。web 版固有の「既定 = 別ウィンドウ」「設定の二層保存」
  「閉じる = 退出」「参加時に毎回選ぶ設定」は**採用しない** (M1 が NO-GO だった場合の web fallback 専用。
  call-window-mode.md 冒頭の降格ステータス参照)。
