...
**状態: 正本** (2026-07-12 制定)。[web-native-parallel.md](web-native-parallel.md) R3「機能パリティは
...
| 画質/FPS ピッカー (配信開始時に 720p/1080p/ソース解像度 × 15/30/60fps) | ✅ | ✅ | `cinny/src/app/features/call/screenShareSettings.ts`。[requirements.md](requirements.md) §3 |
...
| 話者オーバーレイ (発話中ユーザーの表示) | ✅ | ✅ | `cinny/src/app/features/call-status/MemberSpeaking.tsx` |
...
| --- | --- | --- | --- | --- |
| 無再接続ポップアウト (Discord 準拠の窓移動) | ⚠ 再接続あり (別窓に新規 iframe を作り直して再 join) | ✅ 完全無再接続 | ブラウザの iframe を DOM ツリーから別ウィンドウへ移動する Web API は存在せず、`window.open()` で別ウィンドウを作っても中身は新規 iframe になり再接続が必須になる。Electron の WebContentsView は OS ウィンドウ間で再親子付け (付け替え) ができ、通話セッションを維持したまま移動できる | `cinny/src/app/plugins/call/CallPopout.ts` (web の再接続版)。native は [native-milestones.md](native-milestones.md) M3 (2026-07-12 完了、無再接続の窓出し入れ 10 往復切断ゼロを実 E2E で確認) |
...
| 機能 | 想定区分 | 状態 | 根拠 |
| --- | --- | --- | --- |
| 最前面ピン留め (常に最前面表示) | native 限定 (Web API に「他 OS ウィンドウより常に手前に表示する」手段がない。Electron の `setAlwaysOnTop` が前提) | LATER | [native-milestones.md](native-milestones.md) M3「最前面ピン留めは LATER、要否は実機ドッグフーディングで判断」 |
| Stream Deck 等の外部ミュート制御 | native 限定 (グローバルホットキー/USB HID デバイス連携はブラウザのサンドボックスの外) | 検討待ち | [backlog.md](backlog.md) P2、[native-milestones.md](native-milestones.md) M4 |
...
| 話者オーバーレイ右クリックからのユーザー単位音量調整 | 共通コード想定 | 未実装 | [backlog.md](backlog.md) P1 |
| グリッド配信タイルの単体ポップアウト `🗗` | 共通コード基盤 + native は無再接続化 (通話 UI 全体ポップアウトと同じパターンを個別タイルに適用する想定) | 保留 | [backlog.md](backlog.md) P1、[design/call-window-mode.md](../design/call-window-mode.md) |
...
**明示的な非対応 (LATER ではなく不採用)**: 生カスタム CSS の第三者配布 (フィッシング的な見た目偽装・
...
- 未実装/LATER: 11 項目 (うち native 限定想定 4、共通コード想定 5、判断保留 2 [プラグイン基盤・グリッドタイル単体ポップアウトは共通コード基盤 + native 拡張の混在案])
