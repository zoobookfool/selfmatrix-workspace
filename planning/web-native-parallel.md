...
- **実装済み (M2)**: web/native のビルドフラグを分離し、web buildではnative分岐をtree-shakeする。
  CIのweb native-symbol guardを必須ゲートとして維持し、実働テスト環境へnative bridgeを含む配布物を出さない。
...
| カメラ (既定 OFF、設定 opt-in + 各通話で明示 ON) | ✅ | ✅ | 共通コード。設定 ON だけでは capture/publish しない |
...
### R5. デプロイ・ゲート
- **web**: push → CI (typecheck 含む) → immutable SHA tagをGHCRへ生成 → 実働テスト環境で対象SHAを
  pin → `docker compose pull cinny && up -d cinny`。この稼働先は一般公開の本番ではなく動作確認用なので、
  自動ゲートがgreenで対象SHAを確認できた変更は追加のリリース承認を待たず即時反映する。
...
- webの即時反映はmutable tagを許可する意味ではない。常に`sha-<commit>`をpinし、反映前後のimageを追跡可能にする。
