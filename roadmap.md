...
- ~~UI 設計 v1.4 と実装の差分 ①〜④~~ → **すべて実装完了 (2026-07-07、EC 6c692792 +
  ...
  統合 E2E 16 項目 PASS。残課題 (軽微): グリッドタイル化した配信の音量調整ボタン未実装
  (SpotlightTile には有り)、**グリッドの配信タイルに 🗗 (再接続なしのストリーム単体
  ポップアウト) が無い** — v1.4 ①で配信が SpotlightTile から GridTile へ移った副作用。
  注視モードのフォーカス中配信には従来どおり有り。合意 (v1.3「補助機能として維持」) に
  照らすとグリッド側にも追加すべき (2026-07-07 気づき、UI 担当 = GPT へ)、
  強調 strip の見た目の洗練 (実機ドッグフーディングで評価)
  ~~⑤配信の画質/FPS ピッカー (Discord 準拠、v1.4 追加)~~ → **実装完了 (2026-07-07、
  EC d0211a8f + cinny c27d1ef)**。製品経路は cinny の通話コントロール 🖥 → 配信設定
  ポップオーバー (画質 480p/720p/1080p/4K × FPS 15/30/60、既定 4K60) → 共有を開始。
