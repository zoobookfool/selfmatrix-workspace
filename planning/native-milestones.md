...
- **残: electron-builder 同梱化 + リリース CI + 自動更新 (minisign) — フェーズ3
  ([release-pipeline.md](../design/release-pipeline.md) が実装計画)**
...
- **リリース CI 完了 (2026-07-08、desktop 28b41eb)**: `.github/workflows/release.yml` (tag push →
  cinny native + EC embedded ビルド → electron-builder --publish never → Artifact Attestation →
  SHA256SUMS → ドラフトリリース。署名は CI 外)。actionlint clean。RELEASING.md に手順書
- **自前署名による更新検証 完了 (実装、2026-07-08、desktop ecf2f4a/abbdf6e)**: minisign (Ed25519)
  検証を純 Node crypto で実装 (`src/minisign-verify.cjs`。Electron に blake2b512 が無いため純 JS
  BLAKE2b フォールバックも実装、Electron ランタイム probe で担保)。`verifyUpdateCodeSignature` に配線、
  allowDowngrade=false。**残: 運用者の実鍵生成 + 初回実署名時のクロス検証** (release-pipeline のランブック)
- **通話中は更新を保留 完了**: quitAndInstall は callViewState !== "none" の間は呼ばない
  (update-apply-gate.cjs に純関数化 + probe)

**受け入れ状況**: パッケージビルド (electron-builder --dir + フル NSIS) 成立、**packaged exe を
--cinny-shell-smoke で起動し同梱リソース解決 + 製品 config 配信を実測**。Electron セキュリティ MUST
(contextIsolation / preload 最小 / **sandbox:true** / ナビゲーション封じ込め / 契約面ロック) 監査済み。
**M2 実装は完了。残るは運用者のオフライン作業のみ**: (1) minisign 鍵生成 → 実公開鍵を
`RELEASE_PUBLIC_KEY` に埋込 (2) 初回タグ push で CI を実走 → 手元署名 → 検証器クロス確認 → publish
(3) インストーラ実機での接続・自動更新の最終確認。electron-updater の実自動更新は実リリースが要る
ため実装時は probe (外部通信なし) までで、実機確認は初回リリース時。
