...
現在の短期ステータスは [current-status.md](current-status.md)、未完了・保留事項は [backlog.md](backlog.md) を正とします。
この文書は Phase ごとの履歴と大きな計画を残すため、完了済みの実装日誌も含みます。
...
以下は Phase 2b 着手当時 (v1.2) の計画記述であり、履歴として残しています。
当時の未実装差分と実装履歴は本ロードマップの Phase 8「UI 設計 v1.4 と実装の差分」に残しています。
現在の未完了・保留事項は [backlog.md](backlog.md) を正としてください。ポップアウトの主対象は
...
ブランチ構成は [fork-strategy.md](fork-strategy.md) 参照)。typecheck 修正とスパイク成果はコミット済み。
...
運用 runbook (selfmatrix の [docs/operations.md](https://github.com/zoobookfool/selfmatrix/blob/main/docs/operations.md)、荒らし対応・権限・admin API 遮断・外部バックアップ)、
...
**Phase 7 完了 (2026-07-05)。** ⑤ = selfmatrix の [docs/emoji-stickers.md](https://github.com/zoobookfool/selfmatrix/blob/main/docs/emoji-stickers.md) (方針: upstream の MSC2545 実装をそのまま
...
詳細は selfmatrix の [docs/operations.md](https://github.com/zoobookfool/selfmatrix/blob/main/docs/operations.md) §2.1 の注意書きと [requirements.md](requirements.md) §5 を参照)。
...
短期で見るべき未完了リストは [backlog.md](backlog.md) に分離したため、この節は実装・検証の履歴も含む。
...
- ~~EC レビュー指摘の design-mismatch ①②③ と i18n 残り一括~~ → **Wave C/D として完了 (2026-07-06)**。
  ...
  6 項目 PASS
- ~~UI 設計 v1.4 と実装の差分 ①〜④~~ → **すべて実装完了 (2026-07-07、EC 6c692792 +
  cinny b90843e、本番反映済み)**。当時の設計基準は ui-design-notes v1.4 + mocks/ui-mock.html v2.2:
  ①グリッド正方形詰め (arrangeTilesSquare、cols=ceil(√n)) + **視聴オプトイン外は
  タイル化しない** (WatchGate タイル廃止 → 新設の「視聴できる配信」チップバー。視聴中の
  配信は参加者と同じグリッドタイル。ScreenShareTile 新設) ②**強調選択 ON/OFF** (EC の
  emphasis.ts。選択タイルだけでグリッド再構成・選択外は strip・モードは grid のまま・
  spotlight 切替で自動クリア。スポットライト 2 面固定 (split) は削除)。注視モードは
  フォーカス 1 件 + strip クリックで切替 (配信も pin 可能に)。cinny 埋め込みでは EC
  フッター非表示のため、強調選択は cinny の ⋮ メニュー → emphasis_toggle DOM click の
  ブリッジ経由 ③**通話 UI 全体ポップアウト製品化** (CallPopout: hangup→別窓再 join、
  pop-in 往復、ポップアップブロック時は通話維持+エラー表示、popup 所有権ガード)
  ④**初回セットアップ案内** (FirstRunSetup: 初回利用時 1 回のみ、account data フラグ
  in.selfmatrix.first_run_setup でアカウント単位・他デバイス sync 追随、VerificationReminder
  と排他、スキップで Discord 風フォールバック、ミニタイル位置は EC と localStorage 契約)。
  敵対的レビュー 3 視点で major 以上 6 件を検出し修正済み (pin した配信の画質配線・
  grid⇔strip 再マウント・strip 重なり CSS・共有 popup 巻き添え close・pop-in 後の
  ResizeObserver 停止・account data 一発読み)。検証: unit 計 708 (EC 675 + cinny 33) +
  統合 E2E 16 項目 PASS。
  追加対応 (2026-07-07、GPT ローカル作業): グリッドタイル化した配信にも音量 UI を追加し、
  右クリックメニューも停止/ミュート/音量対応に拡張。強調 strip は背景レーン・余白・影を
  足してモック v2.2 の「専用領域の控えめなミニタイル」に寄せた。単独参加中に共有開始を
  押したとき participant 未生成でクリックが失われる既存挙動も、pending 共有リクエストを
  participant 出現時に一度適用する形で修正 (LocalMember test 追加)。また、画面共有が無い
  通話で「ライブ配信中」に見えるステータスは `通話中 {{count}}人` に変更。
  グリッド配信タイルの 🗗 (ストリーム単体ポップアウト) は、別ウィンドウ通話開始モードの
  設計と合わせて再検討するため今回の実装対象から外した。ここまでを
  「別ウィンドウ通話開始モードの検討前」到達点として GitHub に反映し、次の検討対象は
  `通話開始時から CallPopout を生成するモード` とする。
  追加修正 (2026-07-07): チャンネルリストを上下配置にしたとき、設定モーダル内の PageNav が
  誤って横チップ化して「ログアウト」程度しか見えなくなる問題と、Home/Direct/Explore/Inbox/Space
  の横チップ nav でカテゴリ内アイテムが縦に重なり「アドレスで参加」など一部だけ見える問題を修正。
  対応方針は PageNav の横チップ化を main route の明示 opt-in に戻し、横 nav 時だけ NavCategory
  自体も row layout にする。
  ~~⑤配信の画質/FPS ピッカー (Discord 準拠、v1.4 追加)~~ → **実装完了 (2026-07-07、
  EC d0211a8f + cinny c27d1ef)**。製品経路は cinny の通話コントロール 🖥 → 配信設定
  ポップオーバー (画質 720p/1080p/ソース解像度 × FPS 15/30/60、既定 ソース解像度/60)
  → 共有を開始。
  実装の要点: cinny 埋め込みでは EC のフッター UI は非表示 (親が iframe 内の隠しボタンを
  DOM click する方式) のため、選択 UI は cinny 側 (Controls.tsx + screenShareSettings.ts)。
  選択値は同一オリジンの localStorage キー (matrix-setting-screen-share-quality / -fps) で
  EC と共有し、EC 側は開始時に Setting.getStoredValue() で再読込 + 不正値 sanitize、
  captureOptions と publishOptions (画質×FPS 別 maxBitrate 表 + 主レイヤーより低い
  simulcast 低層) に反映。EC フッターの ShareScreenMenuButton は standalone 用に併設。
  検証: unit 計 666 (EC 644 + cinny 22) + 統合 E2E (headless 2 ユーザー) で 720p30 / 4K60 の
  getDisplayMedia 制約反映・永続化・共有中の即停止を確認。
  既知の制限 (既存挙動): 単独参加で SFU 未接続のまま共有開始を押すとクリックが失われる
  (participant 未生成のため no-op。2 人目参加後は正常) — 恒久対応は接続確立後の再試行で、
  SFU 自動再参加の項と合わせて検討
