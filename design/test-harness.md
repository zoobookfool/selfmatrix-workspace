...
- `npm test`: Widget protocol CLI + Web static contract。**`test:web` (Playwright) はここに含まれない** —
  静的検証のみが `npm test` の対象。Widget protocol CLI は electron 非依存の
  `native-prototype/src/widget-bridge-protocol.cjs` (純関数) と preload 実ファイルを使う。2026-07-07 に PASS
  (応答内容検証・`buildWidgetUrl` 経由の origin assert・`sourceIsSelf` 検証込み)
- Electron smoke: 2026-07-07 に Electron 43 runtime で `reparent` / `displayMedia` ともに PASS。evidence JSON はコミット対象
- `npm run test:web`: Playwright UI test。**手動実行**。最終実行: 2026-07-07、2 件 PASS
  (sanitize 済みサマリ: [test-harness/web/evidence/playwright-summary.json](../test-harness/web/evidence/playwright-summary.json)。
  生の `--reporter=json` 出力は個人パスを含むためコミットしない)
...
- カメラ機能が既定 OFF で、OFF 時は UI/video capability/権限要求が閉じている
- 設定 ON だけでは capture/publish せず、参加前または通話中の明示 ON でだけ送信する
- 起動時と参加後にカメラ ON 状態を持ち越さず、音声設定だけではカメラ権限を要求しない
...
electron 非依存の `native-prototype/src/widget-bridge-protocol.cjs` (bridge 実装の純関数群。
`native-prototype/src/main.cjs` はこれを require する薄い委譲) と
`native-prototype/src/widget-bridge-preload.cjs` の実ファイルを使い、preload が IPC へ forwarding
した message を main 側 validation / response policy で確認する。応答 payload の内容
(成功/エラーの別、エラー文言、既知 action の戻り値) と `sourceIsSelf` まで検証する。
