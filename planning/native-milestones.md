...
**M1 完了時の完成度に関する注記 (2026-07-08 Fable 全体レビュー + GPT レビュー)**: 受け入れ条件
(通話コアの技術成立) は満たしたが、以下は **M1 では未検証であり M2 に持ち越す**:
- **実 cinny UI 上でのビデオの表示位置/サイズ追従 (bounds 同期)** は一度も検証していない。E2E は
  WebContentsView 内部のプロトコル/DOM 信号のみを見ており、cinny の実レイアウト (サイドバー開閉・
  チャット表示切替・リサイズ) への追従チャンネルは契約にも実装にも無い (下記 M2 追加項目)。
- 7 語彙のうち **cinny 実 UI のクリックから駆動して検証したのは screenshare のみ**。残り 6 は E2E 専用
  窓口経由で DOM 層のみ検証 (button→RPC の production 配線は E2E 補強で対応中)。
...
- **[MUST] web ビルドの native 分岐無効化** (Fable sec-critical #1 / [web-native-parallel.md](web-native-parallel.md) R2):
  `getSelfmatrixNativeBridge()` をビルド時定数でゲートし web ビルドでは tree-shake。現行 web 本番は
  未対処のため境界つきリスクが残る。**M2 最優先**
- **[MUST] bounds 同期チャンネルの新設** (Fable arch-major #4): cinny の実レイアウト座標
  (`useCallEmbedPlacementSync` が計算) を WebContentsView へ伝える契約 (`updateCallViewBounds` 相当)
  を nativeBridge に追加。現行 `main.cjs` の bounds はハーネス専用固定式で cinny の実 UI に追従しない。
  **実 UI でビデオが正しい位置/サイズに出ることの検証を M2 受け入れに含める**
- **[MUST] mainWindow のナビゲーション封じ込め監査** (Fable sec-critical #2): call view には G7 で
  対処したが、cinny をホストし bridge を持つ mainWindow は未対処 (prototype で暫定対処予定、
  製品構成で監査)。shell window の `sandbox:false`、常時公開の `ensureCallView/detach/attach` 等の
  API 露出面も claim-once / 契約整理の対象 (Fable arch-minor / GPT「次にやるなら #3」)
