...
- host 側に `NativeCallControl` を新設。**public インターフェースは `CallControl` と同一**
  ...
  **RPC 化が必要なのはカテゴリ B のメソッドのみ** (step 2 の精読で確定、2026-07-07):
  `toggleMicrophone` / `toggleVideo` は CallControl.ts 内部でも DOM ではなく
  `call.transport.send(ElementWidgetActions.DeviceMute, ...)` (カテゴリ A = widget action) を
  使っており、(A) の実証済み経路にそのまま乗る。DOM RPC の対象は
  screenshare / spotlight / grid / emphasis / reactions / settings。
  各 B メソッドの中身を `ipcRenderer.invoke('native:call-control:invoke')` → main (解釈しない
  correlationId 中継) → call view preload の RPC に置き換え、状態変化は call view preload →
  IPC push → host が同じ `StateUpdate` を emit する対称構造にする。
  → `useCallControlState` / `CallControls.tsx` は public インターフェースのみに依存しているため
  **無改造で動く**。
  **注意 (step 3 への引き継ぎ)**: spotlight / emphasis は `<input>` checkbox/radio で、`checked` は
  DOM 属性ではなくプロパティのため**属性ベースの MutationObserver では変化を拾えない**。
  web 版 CallControl.ts も click 直後に refreshEmphasisState() で再読込しており、preload 移設版でも
  同じ対策 (click 後の明示再読取り or プロパティ polling) が必要。
  また call view は sandbox:true のため preload から相対 require ができない —
  第 2 preload は `session.registerPreloadScript` で注入する (step 2 で確立)。
...
2. **(B) 単体**: call view preload に `CallControl` 相当の最小移植を実装し、host からの RPC で
   EC 内 DOM が実際に変化 + `StateUpdate` 相当の push が返ることを証跡化。
   **完了 (2026-07-07)** — 逸脱 2 点を記録: (i) 当初例示の「マイクトグル」はカテゴリ A (§2.2 注記)
   のため対象外。(ii) prototype はバックエンド無しで EC が ErrorView を描画するため
   (NativeWidgetDriver が readRoomState 未実装 → Room not found)、対象は ErrorView の
   閉じるボタン。実クリックの独立証明は EC 側 onClick 発火 (`io.element.close` 観測) を
...
| (B) DOM コントロール移設 | 中 (step 2 で高→中) | 経路 (RPC + MutationObserver + registerPreloadScript) は **step 2 で実証済み** (2026-07-07)。mic/video はカテゴリ A と判明し対象面積も縮小。残り: 実 in-call UI での適用 (step 3)、checkbox 系の checked プロパティ対策 (§2.2 注記)。EC の DOM 構造変化 (testid) への依存は web 版と同等のまま |
...
### step 1〜2 レビューで判明した残存リスク (M2 セキュリティ監査への引き継ぎ)

- **`native:call-control:invoke` チャンネル** (step 2 新設): 実コントロール (画面共有等) に
  差し替わると「ユーザー操作なしに他フレームから配信開始を起こせる面」になり得るため、
  送信 API と同じ claim-once に統合済み (step 2 受け入れ修正 F7)。M2 監査で transport と併せて
  公開境界を再設計すること
