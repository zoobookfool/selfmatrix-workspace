...
- リポジトリ作成 + native-prototype からの卒業 — **ローカル完了 (2026-07-08)**:
  ...
  **`spike/native-shell` → product 統合完了 (2026-07-08、cinny 661c7e06)**: 統合ツリーは
  spike とバイト一致 (merge 競合ゼロ) で push 済み。native コードは VITE_SELFMATRIX_NATIVE
  フラグ配下で web ビルドは tree-shake 済み。**これで web 本番も次回デプロイ (自宅 pull) 時に
  リーク修正 + tree-shake が反映される**。config.json のローカル dev 上書きは未コミットのまま
- cinny 同梱 (hashRouter=true 切替、**homeserver は選択制** — 下記参照)、EC 同梱
  - **homeserver 選択制 完了 (2026-07-08、desktop 0ceb61c)**: 製品 config (matrix.org のみ +
    カスタム入力可 + 自サーバー焼き込みゼロ) をモード分岐配信。E2E は dev config 維持。
    cinny 同梱 (sibling 参照 → electron-builder 同梱) は下記 packaging で対応
- **画面共有のソース選択 UI + system audio トグル 完了 (2026-07-08、desktop 3a2d088)**:
  Discord 風ネイティブピッカーウィンドウ (cinny レンダラに画面キャプチャ能力を露出しない設計、
  contractSurfaceGate 維持) + システム音声チェック。E2E で実ピッカー駆動を検証
  (sourceCount=2 / systemAudioTrackAdded=true)。
  ※ fake-ui フラグが getDisplayMedia を短絡する問題を実測で切り分け、
  --auto-accept-camera-and-microphone-capture へ置換して解決
- **About 画面 + AGPL コンプライアンス 完了 (2026-07-08、cinny 5ccf01fe)**: fork 元/変更概要/
  ライセンス、i18n en/ja。web/native 両対応
- **デスクトップ作法 完了 (2026-07-08、desktop 7ac049c/ecfaae6)**: トレイ常駐 (閉じる=最小化・
  右クリックメニュー) / 通知クリックで前面化 / 自動起動 (opt-in・既定 OFF・トレイトグル) /
  CallControl 再マウント耐性 (subtree:true)。未読バッジは LATER
- **electron-builder同梱化 + Product/Release CI + minisign自動更新を実装済み**。
  [release-pipeline.md](../design/release-pipeline.md) は2026-07-12に実装済み正本へ昇格。
...
- **mainWindow のナビゲーション封じ込め** (Fable sec-critical #2): **暫定対応済み**
  ...
  **API 露出面整理 + sandbox も完了 (2026-07-08、desktop 3346787)**: 本番 topology の
  `window.selfmatrixNative` は `claimWidgetTransport` のみ (contractSurfaceGate で恒久回帰
  ゲート化。契約外 4 API は harness トポロジ限定 + 本番では IPC チャンネル未登録の多重防御)。
  mainWindow は `sandbox:true` 化し、実ログイン→WASM→実 LiveKit を E2E で検証済み
- **[SHOULD→完了] CallControl DOM 監視の再マウント耐性** (GPT P2): `ensureBodyObserver()` を
  `subtree:true` 化 + controlElementsChanged で再 observe を要素参照の実変化時に限定 (desktop 7ac049c)
- **[SHOULD→完了] 通話跨ぎの回帰テスト**: callRespawn E2E で join → hangup → 再 join の
  preload 登録 1 維持 / screenshare 単発反転 / claim キャッシュ健在を実測済み (M1 step3c-2/3)
- **デスクトップ作法 完了 (2026-07-08、desktop ecfaae6/7ac049c)**: トレイ常駐 (閉じる=最小化・
  右クリックメニュー・左クリック前面化、--tray-probe + 変異) / 通知クリックで前面化 /
  自動起動 (opt-in・既定 OFF・トレイトグル・レジストリ非汚染)。未読バッジ・ブランドアイコンは LATER
- **About 画面 + AGPL 完了 (2026-07-08、cinny 5ccf01fe)**: fork 元/変更概要/ライセンス、i18n en/ja
- **リリース CI 完了 + 強化 (2026-07-12)**: desktop `product-lock.json`でCinny/ECを完全SHA固定し、
  Cinny側EC lock・実checkout・tag/package versionをビルド前にassert。全Actionsをcommit SHA固定。
  desktop/Cinny/ECのlint・unit・build guard・production auditを通常CIとreleaseで実行し、
  `BUILD-MANIFEST.json` + Attestation + SHA256SUMS + draft releaseを生成する。署名はCI外。
- **自前署名による更新検証 完了 (実経路、2026-07-12)**: minisign (Ed25519)検証とpure JS
  BLAKE2b fallbackに加え、stock `NsisUpdater`の`publisherName`早期return/sidecar未取得を回避する
  `MinisignNsisUpdater`を実装。pending cacheを消してinstaller + `.minisig`を毎回取得し、生成済み
  unpacked製品から実download taskを駆動。正常署名だけ`update-downloaded`、署名欠落/改ざんは
  `ERR_UPDATER_INVALID_SIGNATURE`を確認。実公開鍵は2026-07-09に埋め込み済み。
- **通話中は更新を保留 完了**: quitAndInstall は callViewState !== "none" の間は呼ばない
  (update-apply-gate.cjs に純関数化 + probe)

**受け入れ状況**: フルNSISとpackaged製品の同梱リソース/config、Electron security MUST、実updater
download taskのfail-closed署名検証まで完了。**M2のコード/CI実装は完了**。残る運用ゲートは
(1) 初回tag workflow実走 (2) 実minisign binaryとのクロスチェック + `.minisig`添付 (3) publish後の
旧版→新版自動更新と通話中保留の実機確認。鍵生成と実公開鍵埋め込みは既に完了している。
...
- nativeはメイン/別窓ともECフッターを表示し、窓内で操作を完結させる。窓サイズ/位置は端末ローカルに記憶
  (初期M3では別窓だけ表示していたが、2026-07-14ドッグフーディング後に共通化)
- 最前面ピン留めは当初LATERだったが、2026-07-12に実装済み (`setAlwaysOnTop`、既定OFF・永続化)
...
### M3 完了 (2026-07-12、desktop 5e34e86 + cinny 7bbb17d1)

受け入れ条件「通話中の窓出し入れ 10 往復で切断ゼロ + 別窓クローズでメイン復帰 (通話継続)」を
**実 E2E で成立** (クリーン環境 2 回連続 PASS)。設計・実測詳細は [design/m3-window-ux.md](../design/m3-window-ux.md)。
- ⧉ ポップアウト/ポップイン: WebContentsView 再親子付けで無再接続 (実 ⧉ ボタンクリックで検証)
- **別窓を閉じる = メイン復帰 (通話継続)**: callViewState=attached / callDidNotEnd を実測、変異で回帰ガード実証
- 初期実装は別窓に EC フッター表示 / メイン埋め込みで非表示 (footerVisibilityToggle PASS)。
  2026-07-14に両配置の共通フッターへ改訂 (下記追補)
- 窓サイズ/位置記憶 + 戻り時 bounds 復帰
- 途中で 2 バグ修正 (userData 2 インスタンス衝突 / 戻り時 bounds 未復帰、6718bbf)
- 最前面ピン留めは同日後続コミットで実装済み。Discord実機録画は参考資料として扱う

### M3 UI追補 (2026-07-14)

- nativeのCinny重複通話バーを非表示にし、メイン/別窓ともElement Call共通フッターへ統一。
- 受信音声、画面共有設定、設定、popout/popin、最前面固定、全画面を同じ操作面へ集約。
- Discordに存在しない通話参加/終了タイムライン行を完全非表示。
- 実preload bridgeを通す`m3-window-probe`を追加し、無再接続、WebContents ID不変、明示popin後の
  空窓破棄を確認。desktop全probe green。
- 自動追従PiPは不採用。webの配信単体手動popoutは維持し、native host連携を別機能としてbacklogに残す。

実装入力: Cinny `ffefe11` / Element Call `e662d28` / desktop `095bbe9`。
実アカウント2名の画面共有再確認はM4ドッグフーディング側の運用ゲートとして継続する。
...
- **友達向けインストールガイド骨子 ✅** ([INSTALL-GUIDE.md](https://github.com/zoobookfool/selfmatrix-desktop/blob/main/docs/INSTALL-GUIDE.md)、
